...
import { ArrowLeft, History, User, Clock, Package, ClipboardList, Loader2 } from 'lucide-react'
...
export default function LogsPage() {
    ...
    useEffect(() => {
        // Buscamos as últimas 50 atividades registradas
        const q = query(
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            // Convertemos os documentos e filtramos apenas os que possuem rastro de log
            const docs = snapshot.docs
                .map(doc => ({ id: doc.id, ...doc.data() }))
                .filter((doc: any) => doc.atualizadoEm) // O ": any" resolve o erro de build
                ...
                <div className="space-y-3">
                    {loading ? (
                            ...
                            <div key={log.id} className="bg-[#1e293b] border border-slate-800 p-4 md:p-6 rounded-3xl hover:border-orange-500/40 transition-all group relative overflow-hidden shadow-lg">
                                ...
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex flex-wrap items-center gap-4 mt-2">
                                                <div className="flex items-center gap-1.5 text-[11px] text-slate-400 font-bold">
                                                    ...
                                                    <span className="text-slate-300">{log.atualizadoPor || 'Sistema'}</span>
                                                </div>
                                                {log.os && (
                                    ...
                                    <div className="flex flex-row md:flex-col items-center md:items-end justify-between border-t md:border-t-0 border-slate-800 pt-3 md:pt-0">
                                        <span className={`text-[9px] font-black px-3 py-1 rounded-full mb-1 tracking-widest ${log.tipoAcao === 'EDIÇÃO' ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/20' : 'bg-green-600 text-white shadow-lg shadow-green-900/20'}`}>
                                            {log.tipoAcao || 'CADASTRO'}
                                        </span>
                                        <div className="flex items-center gap-2 text-[10px] text-slate-500 font-mono">
                                            <Clock size={12} />
                                            {log.atualizadoEm ? new Date(log.atualizadoEm).toLocaleString('pt-BR') : '--/--/----'}
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="text-center py-20 bg-[#1e293b]/30 rounded-[3rem] border-2 border-dashed border-slate-800">
                            ...
                            <p className="text-[10px] text-slate-700 mt-2 uppercase italic">Altere ou cadastre algo na planilha para ver os logs aqui.</p>
                        </div>
                    )}
                </div>
            </div>
